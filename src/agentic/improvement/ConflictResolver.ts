...
export class ConflictResolver {
   ...
   */
  async auditRuleConflicts(): Promise<string[]> {
    console.log(
      '[ConflictResolver] Auditing cognitive rules for conflicts and overlaps...',
    )
    ...
    const seen = new Map<string, CognitiveRule>()
    for (const rule of rules) {
      ...
      if (seen.has(key)) {
        ...
        if (existing.action !== rule.action) {
          conflicts.push(
            `Direct Conflict: Multiple actions for '${key}' ('${existing.action}' vs '${rule.action}')`,
          )
        } else if (existing.condition === rule.condition) {
          conflicts.push(
            `Redundant Rules: Duplicate rule detected for '${key}' with same condition.`,
          )
        }
      }
      seen.set(key, rule)
        ...
        for (let i = 0; i < list.length; i++) {
          for (let j = i + 1; j < list.length; j++) {
            ...
            if (r1.condition && r2.condition) {
              ...
              if (similarity > 0.6) {
                conflicts.push(
                  `Potential Semantic Overlap on '${table}': Rules ${r1.id} and ${r2.id} have high condition similarity (${(similarity * 100).toFixed(1)}%)`,
                )
              }
            }
          }
