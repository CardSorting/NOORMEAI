...
export class SelfTestRegistry {
     ...
     */
    async registerProbe(name: string, script: string, expectedOutcome?: string): Promise<void> {
        ...
        await this.db
            ...
            .onConflict((oc: any) => oc.column('name').doUpdateSet({
                script,
                expected_outcome: expectedOutcome
            }))
            .execute()
     ...
     */
    async runAllProbes(): Promise<{ name: string, success: boolean, error?: string }[]> {
                    ...
                    switch (action) {
                        ...
                        case 'check_data_integrity':
                            ...
                            const orphanedKnowledge = await this.db
                                ...
                                .where((eb: any) => eb.not(
                                    eb.exists(
                                        eb.selectFrom(sessionsTbl as any)
                                            .select('id')
                                            .whereRef(`${knowledgeTable}.source_session_id` as any, '=', `${sessionsTbl}.id` as any)
                                    )
                                ))
                                .execute()
                            ...
                            const orphanedActions = await this.db
                                ...
                                .where((eb: any) => eb.not(
                                    eb.exists(
                                        eb.selectFrom(sessionsTbl as any)
                                            .select('id')
                                            .whereRef(`${actionsTable}.session_id` as any, '=', `${sessionsTbl}.id` as any)
                                    )
                                ))
                                .execute()

                            success = orphanedKnowledge.length === 0 && orphanedActions.length === 0
                            break
