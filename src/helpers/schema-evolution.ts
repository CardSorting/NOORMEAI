...
export class SchemaEvolutionHelper {
  ...
  async applySuggestion(ddl: string): Promise<void> {
    ...
    for (const id of identifiers) {
      ...
      try {
        ...
        if (
          !['VARCHAR', 'TIMESTAMP', 'INT', 'TEXT'].some((t) => upId.includes(t))
        ) {
          console.warn(
            `[SchemaEvolution] Identifier validation warning for "${id}":`,
            e,
          )
        }
      }
  ...
  private normalizeKyselyToSql(jsCode: string): string {
    ...
    const addColumnMatch = jsCode.match(
    ...
    if (createTableMatch) {
      ...
      const cols: string[] = []
      const colMatches = jsCode.matchAll(
        /\.addColumn\(['"](.*?)['"],\s*['"](.*?)['"](?:,.*?)?\)/g,
      )
