...
global.createMockNOORMME = () => {
  ...
  return {
    ...
    getSQLiteOptimizations: jest.fn(() =>
      Promise.resolve({
    ...
    getSQLiteIndexRecommendations: jest.fn(() =>
      Promise.resolve({
        recommendations: [
          {
            table: 'users',
            column: 'created_at',
            reason: 'Frequently queried for sorting',
            impact: 'high',
          },
          {
            table: 'posts',
            column: 'created_at',
            reason: 'Used in date range queries',
            impact: 'medium',
          },
        ],
      }),
    ),
    getSQLitePerformanceMetrics: jest.fn(() =>
      Promise.resolve({
        cacheHitRate: 0.85,
    ...
    getMigrationManager: jest.fn(() => ({
      getMigrationStatus: jest.fn(() =>
        Promise.resolve({
          ...
          pendingMigrations: [],
          availableMigrations: [{ version: '001', name: 'initial_schema' }],
        }),
      ),
