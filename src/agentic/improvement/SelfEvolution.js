...
class SelfEvolution {
    ...
    constructor(db, config) {
        ...
        this.snapshotsTable = config.agentic?.metadata?.snapshotsTable || 'agent_snapshots';
    }
    /**
    ...
    async ensureTelemetryColumns() {
        ...
        for (const table of tables) {
            try {
                ...
                const tableInfo = await introspector.getTables().then(ts => ts.find(t => t.name === table));
                if (tableInfo && !tableInfo.columns.some(c => c.name === 'accessed_at')) {
     ...
     */
    async setupTelemetryTables() {
        const eventsTable = this.config.agentic?.telemetryEventsTable || 'agent_telemetry_events';
        const evolutionTable = this.config.agentic?.sessionEvolutionTable || 'agent_session_evolution';
        const metricsTable = this.config.agentic?.researchMetricsTable || 'agent_research_metrics';
        ...
        await this.db.schema
            ...
            .addColumn('created_at', 'timestamp', (col) => col.defaultTo((0, sql_js_1.sql) `CURRENT_TIMESTAMP`))
            .execute();
    ...
    async regenerateTypes() {
        ...
        if (this.config.agentic?.metadata?.typesOutputPath) {
            ...
            await fs.writeFile(outputPath, generated.interfaces + '\n' + generated.types);
        }
    }
    /**
     ...
     */
    async proactivelyOptimize(context) {
    ...
    async rollbackToSnapshot(name) {
        ...
        if (metadata.ddl) {
            ...
            if (inverse) {
                ...
                await sql_js_1.sql.raw(inverse).execute(this.db);
                await this.regenerateTypes();
            ...
            throw new Error(`Snapshot ${name} has no DDL metadata for automated inversion.`);
        }
    }
    invertDDL(ddl) {
        ...
        const createTableMatch = d.match(/CREATE TABLE\s+(?:IF NOT EXISTS\s+)?([^\s\()]+)/i);
        if (createTableMatch) {
        ...
        const createIndexMatch = d.match(/CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF NOT EXISTS\s+)?([^\s\(]+)/i);
        if (createIndexMatch) {
            return `DROP INDEX IF EXISTS ${createIndexMatch[1]}`;
        }
        // Match patterns like: ALTER TABLE "table_name" ADD [COLUMN] "column_name"
        const addColumnMatch = d.match(/ALTER TABLE\s+([^\s]+)\s+ADD\s+(?:COLUMN\s+)?([^\s\(\)]+)\s+([^\s]+)/i);
        if (addColumnMatch) {
...
exports.SelfEvolution = SelfEvolution;
