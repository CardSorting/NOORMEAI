...
export function compareSchemas(
  ...
  const tablesRemoved = differences.filter(
    (d) => d.type === 'table_removed',
  ).length
...
export function generateSyncSQL(
  ...
  for (const diff of comparison.differences) {
    switch (diff.type) {
      ...
      case 'table_removed':
        sqlStatements.push(
          `-- WARNING: Table '${diff.table}' should be dropped (manual action required)`,
        )
        break

      case 'column_removed':
        sqlStatements.push(
          `-- WARNING: Column '${diff.table}.${diff.column}' should be dropped (manual action required)`,
        )
        break
    }
  }
