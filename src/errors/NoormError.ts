...
export class ColumnNotFoundError extends NoormError {
  constructor(
    ...
    const similarColumns = availableColumns.filter(
      (col) =>
  ...
  private static calculateSimilarity(str1: string, str2: string): number {
    ...
    const editDistance = ColumnNotFoundError.levenshteinDistance(
      longer,
      shorter,
    )
  ...
  constructor(originalError: Error, databasePath: string) {
    super(
      `Failed to initialize database at ${databasePath}: ${originalError.message}`,
      {
        ...
        suggestion:
          'Check database permissions, path validity, and connection settings',
        originalError,
      },
    )
...
export class ValidationError extends NoormError {
  constructor(message: string, validationIssues: string[] = []) {
    super(message, {
      ...
      suggestion:
        'Check your input data and ensure it matches the expected schema',
  ...
  constructor(tableName: string, originalError: Error) {
    super(
      `Failed to discover schema for table '${tableName}': ${originalError.message}`,
      {
        ...
        suggestion:
          'Check table permissions, table name validity, or database connection',
        originalError,
      },
    )
  ...
  constructor(
    ...
    super(
      `Migration '${migrationName}' failed${context?.step ? ` at step: ${context.step}` : ''}: ${originalError.message}`,
      {
        operation: 'migration',
        suggestion:
          'Check migration SQL syntax, database permissions, or rollback the migration',
        originalError,
      },
    )
  ...
  constructor(tableName: string, originalError: Error) {
    super(
      `Failed to generate types for table '${tableName}': ${originalError.message}`,
      {
        ...
        suggestion:
          'Check table schema, column types, or custom type mappings configuration',
        originalError,
      },
    )
...
function findSimilarColumns(columns: string[], target: string): string[] {
  ...
  const substringMatches = columns.filter(
    (col) =>
      col.toLowerCase().includes(lowerTarget) ||
