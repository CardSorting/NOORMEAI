...
class SQLiteConstraintDiscovery {
    ...
    async getForeignKeyInfo(db, tableName) {
        try {
            const result = await (0, sql_js_1.sql) `PRAGMA foreign_key_list(${sql_js_1.sql.lit(tableName)})`.execute(db);
    ...
    async isForeignKeySupportEnabled(db) {
        try {
            const result = await (0, sql_js_1.sql) `PRAGMA foreign_keys`.execute(db);
    ...
    async checkOrphanedRecords(db, table, foreignKey) {
        ...
        FROM ${sql_js_1.sql.id(table)} t
        LEFT JOIN ${sql_js_1.sql.id(foreignKey.referencedTable)} r
          ON t.${sql_js_1.sql.id(foreignKey.column)} = r.${sql_js_1.sql.id(foreignKey.referencedColumn)}
        WHERE r.${sql_js_1.sql.id(foreignKey.referencedColumn)} IS NULL
      `.execute(db);
                ...
                for (const table of tables) {
                    ...
                    for (const fk of foreignKeys) {
                        ...
                        if (!hasIndex) {
                            ...
                            if (!dryRun) {
                                try {
                                    await sql_js_1.sql.raw(indexSQL).execute(db);
                ...
                for (const table of tables) {
                    ...
                    for (const fk of foreignKeys) {
                        ...
                        if (orphanedCount > 0) {
                            ...
                            if (!dryRun) {
                                try {
                                    await sql_js_1.sql.raw(cleanupSQL).execute(db);
...
exports.SQLiteConstraintDiscovery = SQLiteConstraintDiscovery;
