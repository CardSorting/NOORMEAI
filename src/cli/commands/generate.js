...
async function generate(options = {}) {
    ...
    try {
        ...
        if (!options.reposOnly) {
            ...
            await fs_1.promises.writeFile(typesPath, typesContent);
            generatedFiles.push(typesPath);
            console.log(chalk_1.default.green(`✅ Generated TypeScript types: ${typesFile}`));
        }
        ...
        if (!options.typesOnly) {
            ...
            await fs_1.promises.writeFile(reposPath, reposContent);
            generatedFiles.push(reposPath);
            console.log(chalk_1.default.green(`✅ Generated repository classes: repositories.ts`));
        }
        ...
        console.log(chalk_1.default.green(`✅ Generated usage examples: usage-examples.ts`));
        ...
        console.log(chalk_1.default.gray('2. Use the repository classes for type-safe database operations'));
        console.log(chalk_1.default.gray('3. Configure automation settings in automation.config.ts'));
        console.log(chalk_1.default.gray('4. Check usage-examples.ts for implementation patterns'));
        await db.close();
    }
    catch (error) {
        console.error(chalk_1.default.red('❌ Code generation failed:'), error instanceof Error ? error.message : error);
        process.exit(1);
    }
}
...
function generateRepositoryType(table) {
    const tableName = pascalCase(table.name);
...
function generateAutomationConfig(tables) {
...
function generateUsageExamples(tables) {
