...
export class SessionManager {
     ...
     */
    async createSession(name?: string, metadata?: Record<string, any>): Promise<AgentSession> {
    ...
    async deleteSession(id: string | number): Promise<void> {
        await this.db.transaction().execute(async (trx) => {
            ...
            await trx.deleteFrom(this.messagesTable as any).where('session_id', '=', id).execute()
            await trx.deleteFrom(this.goalsTable as any).where('session_id', '=', id).execute()
            await trx.deleteFrom(this.memoriesTable as any).where('session_id', '=', id).execute()

            // Delete the session itself
            await trx.deleteFrom(this.sessionsTable as any).where('id', '=', id).execute()
        })
    }
     ...
     */
    async getHistory(sessionId: string | number, limit: number = 50): Promise<AgentMessage[]> {
    ...
    async upsertGoal(
        sessionId: string | number,
        description: string,
        options: { status?: AgentGoal['status'], priority?: number, parentId?: string | number, metadata?: Record<string, any> } = {}
    ): Promise<AgentGoal> {
     ...
     */
    async getGoals(sessionId: string | number, status?: AgentGoal['status']): Promise<AgentGoal[]> {
