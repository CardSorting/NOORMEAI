...
export class SchemaWatcher {
  ...
  async startWatching(): Promise<void> {
    ...
    this.logger.info(
      `Schema watcher started (polling every ${this.options.pollInterval}ms)`,
    )
  }
   ...
   */
  removeSchemaChangeCallback(
    callback: (changes: SchemaChange[]) => void,
  ): void {
  ...
  async checkForChanges(): Promise<SchemaChange[]> {
    try {
      ...
      // Debug logging
      this.logger.debug(
        `Schema check - Last hash: ${this.lastSchemaHash}, Current hash: ${currentHash}, Tables: ${currentSchema.tables.length}`,
      )
  ...
  private hashSchema(schema: SchemaInfo): string {
    ...
    const schemaString = JSON.stringify({
      ...
      relationships: schema.relationships.sort((a, b) =>
        `${a.fromTable}.${a.name}`.localeCompare(`${b.fromTable}.${b.name}`),
      ),
    })
