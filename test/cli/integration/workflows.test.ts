...
describe('CLI Integration Workflows', () => {
  ...
  describe('Complete initialization workflow', () => {
    it('should handle full project initialization workflow', async () => {
      ...
      const initResult = await runCLICommand(
        'npx noormme init --database ./test.sqlite --force --auto-optimize --auto-index',
        {
      ...
      const dbFileExists = await fs
        .access(path.join(testContext.tempDir, 'lib', 'db.ts'))
        .then(() => true)
        .catch(() => false)
      const packageJsonExists = await fs
        .access(path.join(testContext.tempDir, 'package.json'))
        .then(() => true)
        .catch(() => false)
      ...
      const typesExists = await fs
        .access(path.join(testContext.tempDir, 'generated', 'database.d.ts'))
        .then(() => true)
        .catch(() => false)
      const reposExists = await fs
        .access(path.join(testContext.tempDir, 'generated', 'repositories.ts'))
        .then(() => true)
        .catch(() => false)
  ...
  describe('Development workflow', () => {
    it('should handle typical development workflow', async () => {
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'dev.sqlite'),
        'mock database',
      )

      // Inspect database
      const inspectResult = await runCLICommand(
        'npx noormme inspect --relationships --optimizations',
        {
          cwd: testContext.tempDir,
      ...
      const generateResult = await runCLICommand(
        'npx noormme generate --types-only',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './dev.sqlite' },
        },
      ...
      // Optimize
      const optimizeResult = await runCLICommand(
        'npx noormme optimize --pragma --indexes',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './dev.sqlite' },
        },
  ...
  describe('Migration workflow', () => {
    it('should handle migration workflow', async () => {
      // Initialize project
      await runCLICommand(
        'npx noormme init --database ./migrate.sqlite --force',
        {
          cwd: testContext.tempDir,
        },
      )
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'migrate.sqlite'),
        'mock database',
      )
      ...
      const generateResult = await runCLICommand(
        'npx noormme migrate --generate add_user_profile',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './migrate.sqlite' },
        },
      ...
      expect(generateResult.stdout).toContain(
        'Generating Migration: add_user_profile',
      )

      // Migrate to latest
      const migrateResult = await runCLICommand(
  ...
  describe('Monitoring workflow', () => {
    it('should handle monitoring and analysis workflow', async () => {
      // Initialize project
      await runCLICommand(
        'npx noormme init --database ./monitor.sqlite --force',
        {
          cwd: testContext.tempDir,
        },
      )
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'monitor.sqlite'),
        'mock database',
      )

      // Analyze query patterns
      const analyzeResult = await runCLICommand(
        'npx noormme analyze --patterns --slow-queries',
        {
          cwd: testContext.tempDir,
      ...
      const statusResult = await runCLICommand(
        'npx noormme status --metrics --optimizations --cache',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './monitor.sqlite' },
        },
      ...
      const watchResult = await runCLICommand(
        'timeout 5s npx noormme watch --interval 1000',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './monitor.sqlite' },
        },
  ...
  describe('Environment variable workflow', () => {
    it('should use environment variables consistently', async () => {
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'env.sqlite'),
        'mock database',
      )
  ...
  describe('Package.json scripts workflow', () => {
    it('should work with package.json scripts', async () => {
      // Initialize project
      await runCLICommand(
        'npx noormme init --database ./scripts.sqlite --force',
        {
          cwd: testContext.tempDir,
        },
      )
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'scripts.sqlite'),
        'mock database',
      )
  ...
  describe('Configuration workflow', () => {
    it('should handle different configuration options', async () => {
      ...
      await runCLICommand(
        'npx noormme init --database ./config.sqlite --force --no-auto-optimize --no-auto-index',
        {
          cwd: testContext.tempDir,
        },
      )
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'config.sqlite'),
        'mock database',
      )

      // Check that configuration was applied
      const dbFileContent = await fs.readFile(
        path.join(testContext.tempDir, 'lib', 'db.ts'),
        'utf8',
      )
      ...
      const generateResult = await runCLICommand(
        'npx noormme generate --format ts --output typescript',
        {
          cwd: testContext.tempDir,
      ...
      const typesExists = await fs
        .access(path.join(testContext.tempDir, 'typescript', 'database.ts'))
        .then(() => true)
        .catch(() => false)
      expect(typesExists).toBe(true)
    })
  ...
  describe('Performance testing workflow', () => {
    it('should handle performance analysis workflow', async () => {
      ...
      await fs.writeFile(
        path.join(testContext.tempDir, 'perf.sqlite'),
        'mock database',
      )

      // Run comprehensive performance analysis
      const analyzeResult = await runCLICommand(
        'npx noormme analyze --patterns --slow-queries --indexes --report',
        {
          cwd: testContext.tempDir,
      ...
      // Apply optimizations
      const optimizeResult = await runCLICommand(
        'npx noormme optimize --pragma --indexes --analyze --wal',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './perf.sqlite' },
        },
      ...
      const statusResult = await runCLICommand(
        'npx noormme status --metrics --optimizations',
        {
          cwd: testContext.tempDir,
          env: { DATABASE_PATH: './perf.sqlite' },
        },
