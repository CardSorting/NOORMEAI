...
class MetricsCollector {
     ...
     */
    async load() {
        if (!this.config.persistPath || !this.config.enabled)
            return;
        try {
     ...
     */
    recordQuery(query, executionTime, options = {}) {
 ...
 */
function createMetricsCollector(config, logger) {
