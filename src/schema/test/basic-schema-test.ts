...
async function runTests() {
  ...
  test.test('DiscoveryFactory should be a singleton', () => {
    try {
      const {
        DiscoveryFactory,
      } = require('../core/factories/discovery-factory.js')
  ...
  test.test('SchemaDiscoveryCoordinator should be a singleton', () => {
    try {
      ...
      } = require('../core/coordinators/schema-discovery.coordinator.js')
      const instance1 = SchemaDiscoveryCoordinator.getInstance()
      const instance2 = SchemaDiscoveryCoordinator.getInstance()
      test.expect(instance1).toBe(instance2)
    } catch (error) {
      console.log(
        '⚠️  SchemaDiscoveryCoordinator test skipped (module not available)',
      )
    }
  })
  ...
  test.test('SQLiteDiscoveryCoordinator should be a singleton', () => {
    try {
      ...
      } = require('../dialects/sqlite/sqlite-discovery.coordinator.js')
      const instance1 = SQLiteDiscoveryCoordinator.getInstance()
      const instance2 = SQLiteDiscoveryCoordinator.getInstance()
      test.expect(instance1).toBe(instance2)
    } catch (error) {
      console.log(
        '⚠️  SQLiteDiscoveryCoordinator test skipped (module not available)',
      )
    }
  })
  ...
  test.test('DiscoveryFactory should support PostgreSQL and SQLite', () => {
    try {
      ...
      console.log(
        '⚠️  Factory dialect support test skipped (module not available)',
      )
    }
  })
  ...
  test.test(
    'DiscoveryFactory should throw error for unsupported dialects',
    () => {
      try {
        const {
          DiscoveryFactory,
  ...
  test.test('PostgreSQL should have correct capabilities', () => {
    try {
      ...
      console.log(
        '⚠️  PostgreSQL capabilities test skipped (module not available)',
      )
    }
  })
