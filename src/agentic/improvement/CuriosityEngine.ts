...
export class CuriosityEngine {
  ...
  async identifyKnowledgeGaps(): Promise<
    {
      entity: string
      type: 'low_confidence' | 'contradiction' | 'unverified'
      details: string
    }[]
  > {
    console.log(
      '[CuriosityEngine] Analyzing knowledge base for gaps and contradictions...',
    )
   ...
   */
  async identifyKnowledgeHotspots(): Promise<
    { entity: string; references: number; density: number; gap: string }[]
  > {
    ...
    const topEntities = (await this.db
      ...
      .select([
        ...
        (eb: any) => eb.fn.sum('metric_value').as('references'),
      ])
  ...
  async suggestQuestions(entity: string): Promise<string[]> {
    ...
    const lowConfidenceItems = (knowledge as any[]).filter(
      (k) => k.confidence < 0.6,
    )
    if (lowConfidenceItems.length > 0) {
      questions.push(
        `Verify the accuracy of recent claims about ${entity}, specifically regarding "${lowConfidenceItems[0].fact}".`,
      )
    }
    ...
    if (allTags.has('performance')) {
      questions.push(
        `Are there any known latency bottlenecks or resource constraints for ${entity}?`,
      )
    }
    if (allTags.has('security')) {
      questions.push(
        `What are the authentication and authorization protocols protecting ${entity}?`,
      )
    }
    ...
    if (knowledge.length > 1) {
      questions.push(
        `Resolve the discrepancy between competing facts about ${entity} to establish a single source of truth.`,
      )
    }
  ...
  async proposeResearch(): Promise<string[]> {
    ...
    if (evolutions > 5) {
      proposals.push(
        `[SOVEREIGN RITUAL] The engine has undergone ${evolutions} structural evolutions this week. Consider a 'Sovereign Publication Ritual' to update the registry.`,
      )
    }
