...
export class CognitiveRepository<T> {
  ...
  private async triggerRules(
    ...
    const result = await this.cortex.rules.evaluateRules(
      this.table.name,
      operation,
      data,
    )
    ...
    if (result.action === 'deny') {
      throw new NoormError(
        `Operation ${operation} on ${this.table.name} denied: ${result.reason}`,
        {
    ...
    if (result.action === 'mask' && result.ruleId) {
      const rules = await this.cortex.rules.getActiveRules(
        this.table.name,
        operation,
      )
      const rule = rules.find((r) => r.id === result.ruleId)
      if (rule) {
        return this.cortex.rules.applyMasking(
