...
import type { AgenticConfig, AgentMessage, AgentEpoch } from '../types/index.js'
...
export class SessionCompressor {
  ...
  async compress(
    ...
    console.log(
      `[SessionCompressor] Compressing session ${sessionId} with ${anchors.length} semantic anchors.`,
    )
   ...
   */
  async semanticPruning(
    sessionId: string | number,
    keepAnchors: boolean = true,
  ): Promise<number> {
    console.log(
      `[SessionCompressor] Performing semantic pruning for session ${sessionId}...`,
    )
    ...
    console.log(
      `[SessionCompressor] Pruned ${count} non-anchor messages from session ${sessionId}`,
    )
    return count
  }
  ...
  async consolidateEpochsIntoEra(sessionId: string | number): Promise<void> {
    ...
    if (epochs.length > 10) {
      console.log(
        `[SessionCompressor] Consolidating ${epochs.length} epochs into a new Era for session ${sessionId}`,
      )

      const summary =
        `Consolidated era summary of ${epochs.length} epochs: ` +
        epochs
          .slice(0, 5)
          .map((e) => e.summary)
          .join('; ') +
