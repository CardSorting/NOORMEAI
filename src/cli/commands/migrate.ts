  ...
  try {
    ...
    if (options.status || (!options.to && !options.latest && !options.rollback)) {
      ...
      try {
        ...
        console.log(chalk.gray(`  Current version: ${status.lastMigration || 'None'}`))
        console.log(chalk.gray(`  Available migrations: ${status.totalMigrations}`))
        ...
        if (status.appliedList.length > 0) {
          ...
          status.appliedList.forEach((migration: any, index: number) => {
            console.log(chalk.gray(`  ${index + 1}. ${migration.name} (Applied: ${migration.applied_at})`))
          })
        }
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.optimizationsApplied.length > 0) {
            console.log(chalk.blue(`🔧 Applied ${result.optimizationsApplied.length} SQLite optimizations.`))
          }
    ...
    if (options.rollback) {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log(chalk.gray(`Rolled back record for: ${result.migrationName}`))
        } else {
