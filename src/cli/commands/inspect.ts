...
export async function inspect(
  ...
  try {
    ...
    if (tableName) {
      ...
      console.log(
        chalk.green(
          `📊 Discovered ${schemaInfo.tables.length} tables with complete automation:\n`,
        ),
      )
...
async function getTableRowCount(
  ...
  try {
    const introspector = (db.getKysely() as any)
      .getExecutor()
...
async function showTableOptimizations(
  table: TableInfo,
  db: NOORMME,
): Promise<void> {
  try {
    ...
    const tableRecommendations = indexRecs.recommendations.filter(
      (rec: any) => rec.table === table.name,
    )

    if (tableRecommendations.length > 0) {
      console.log(
        chalk.yellow(
          `💡 ${tableRecommendations.length} optimization recommendations:`,
        ),
      )
      tableRecommendations.forEach((rec: any, index: number) => {
        console.log(
          chalk.gray(
            `  ${index + 1}. ${rec.column}: ${rec.reason} (${rec.impact} impact)`,
          ),
        )
      })
    } else {
      console.log(
        chalk.green('✅ No optimization recommendations for this table'),
      )
    }
    ...
    console.error(
      chalk.red('❌ Failed to get optimization analysis:'),
      error instanceof Error ? error.message : error,
    )
  }
}

async function showTableIndexAnalysis(
  table: TableInfo,
  db: NOORMME,
): Promise<void> {
  try {
    ...
    if (table.indexes.length > 0) {
      ...
      table.indexes.forEach((index, i) => {
        ...
        console.log(
          chalk.gray(
            `  ${i + 1}. ${index.name} (${type}): ${index.columns.join(', ')}`,
          ),
        )
      })
    } else {
      console.log(
        chalk.yellow(
          '⚠️ No indexes found - consider adding indexes for frequently queried columns',
        ),
      )
    }
    ...
    if (tableRecommendations.length > 0) {
      ...
      tableRecommendations.forEach((rec: any, index: number) => {
        console.log(
          chalk.gray(
            `  ${index + 1}. CREATE INDEX idx_${rec.table}_${rec.column} ON ${rec.table}(${rec.column});`,
          ),
        )
    ...
    console.error(
      chalk.red('❌ Failed to get index analysis:'),
      error instanceof Error ? error.message : error,
    )
  }
}

async function showTablePerformanceMetrics(
  table: TableInfo,
  db: NOORMME,
): Promise<void> {
  try {
...
async function showOptimizationOverview(db: NOORMME): Promise<void> {
  try {
    ...
    console.log(
      chalk.green(
        `✅ Applied optimizations: ${optimizations.appliedOptimizations.length}`,
      ),
    )
    if (optimizations.appliedOptimizations.length > 0) {
      optimizations.appliedOptimizations.forEach(
    ...
    console.error(
      chalk.red('❌ Failed to get optimization overview:'),
      error instanceof Error ? error.message : error,
    )
  }
}
...
async function showIndexAnalysis(db: NOORMME): Promise<void> {
  try {
    ...
    if (indexRecs.recommendations.length > 0) {
      console.log(
        chalk.yellow(
          `💡 ${indexRecs.recommendations.length} index recommendations available:`,
        ),
      )
      ...
      if (indexRecs.recommendations.length > 10) {
        console.log(
          chalk.gray(
            `  ... and ${indexRecs.recommendations.length - 10} more recommendations`,
          ),
        )
      }
    } else {
...
async function showPerformanceOverview(db: NOORMME): Promise<void> {
  try {
    ...
    console.log(
      chalk.gray(`Cache hit rate: ${(metrics.cacheHitRate * 100).toFixed(1)}%`),
    )
    console.log(
      chalk.gray(
    ...
    console.log(
      chalk.gray(`WAL mode: ${metrics.walMode ? 'Enabled' : 'Disabled'}`),
    )
    console.log(
      chalk.gray(
        `Foreign keys: ${metrics.foreignKeys ? 'Enabled' : 'Disabled'}`,
      ),
    )
  } catch (error) {
    console.error(
      chalk.red('❌ Failed to get performance overview:'),
      error instanceof Error ? error.message : error,
    )
  }
}

async function showAutomationRecommendations(
  schemaInfo: any,
  db: NOORMME,
): Promise<void> {
  try {
    ...
    const tablesWithoutIndexes = schemaInfo.tables.filter(
      (table: any) => table.indexes.length === 0 && table.columns.length > 1,
    )
    if (tablesWithoutIndexes.length > 0) {
      recommendations.push(
        `Consider adding indexes to tables: ${tablesWithoutIndexes.map((t: any) => t.name).join(', ')}`,
      )
    }

    // Check for missing foreign keys
    const tablesWithIdColumns = schemaInfo.tables.filter(
      (table: any) =>
        table.columns.some((col: any) => col.name.includes('_id')) &&
        table.foreignKeys.length === 0,
    )
    if (tablesWithIdColumns.length > 0) {
      recommendations.push(
        `Add foreign key constraints to tables: ${tablesWithIdColumns.map((t: any) => t.name).join(', ')}`,
      )
    }
    ...
    console.error(
      chalk.red('❌ Failed to get automation recommendations:'),
      error instanceof Error ? error.message : error,
    )
  }
}
...
function showTableDetails(
  ...
  if (table.indexes.length > 0) {
    ...
    table.indexes.forEach((idx) => {
      ...
      console.log(
        chalk.gray(`  ${idx.name} (${type}): ${idx.columns.join(', ')}`),
      )
    })
  }
  ...
  if (tableRelationships.length > 0) {
    ...
    tableRelationships.forEach((rel) => {
      ...
      console.log(
        chalk.gray(`  ${rel.name} (${type}) ${direction} ${otherTable}`),
      )
    })
  }
  ...
  console.log(
    chalk.gray(`const ${table.name}Repo = db.getRepository('${table.name}')`),
  )
  ...
  if (tableRelationships.length > 0) {
    ...
    console.log(
      chalk.gray(
        `const withRelations = await ${table.name}Repo.findWithRelations(1, [${relationshipNames}])`,
      ),
    )
  }
