...
class PostgresIntrospector extends database_introspector_js_1.DatabaseIntrospector {
    #db;
    constructor(db) {
        super(db);
        this.#db = db;
    ...
    async getSchemas() {
        const schemas = await (0, sql_js_1.sql) `
      SELECT schema_name
    ...
    async getColumns(tableName) {
        const columns = await (0, sql_js_1.sql) `
      SELECT
        c.column_name,
        c.ordinal_position,
        c.column_default,
    ...
    async getIndexes(tableName) {
        const indexes = await (0, sql_js_1.sql) `
      SELECT
    ...
    async getForeignKeys(tableName) {
        const foreignKeys = await (0, sql_js_1.sql) `
      SELECT
        tc.constraint_name,
        tc.table_name,
        kcu.column_name,
    ...
    async #getTableMetadata(schema, options) {
        // Get all tables in the schema
        let tablesQuery = (0, sql_js_1.sql) `
    ...
    `.execute(this.#db);
        ...
        return tables.rows.map((table) => ({
            ...
            foreignKeys: this.#parseForeignKeys(foreignKeysByTable.get(table.table_name) ?? []),
        }));
    }
    ...
    #parseForeignKeys(rawForeignKeys) {
