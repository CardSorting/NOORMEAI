...
export class VectorIndexer {
  ...
  async search(
    ...
    if (this.config.provider === 'pgvector') {
      let query = this.typedDb
        ...
        .selectAll()
        .orderBy((eb: any) =>
          eb.raw(`embedding <=> '[${embedding.join(',')}]'::vector`),
        )
        .limit(limit)
      ...
      try {
        const result = await this.db
          ...
          .where((eb: any) =>
            eb.raw(
              `rowid IN (SELECT rowid FROM vss_${this.memoriesTable} WHERE embedding_column = ? LIMIT ?)`,
        ...
        console.warn(
          '[VectorIndexer] SQLite-vss search failed, falling back to manual search:',
          e,
        )
      }
    }
  ...
  private async keywordSearch(
    keyword: string,
    options: { limit: number; sessionId?: string | number },
  ): Promise<AgentMemory[]> {
    ...
    if (this.config.provider === 'sqlite-vss') {
      ...
      try {
        query = query.where((eb: any) =>
          eb.raw(
            `rowid IN (SELECT rowid FROM ${this.memoriesTable}_fts WHERE ${this.memoriesTable}_fts MATCH ?)`,
            [keyword],
          ),
        )
      } catch (e) {
  ...
  private mergeWithRRF(
    ...
    const scores: Record<
      string | number,
      { score: number; memory: AgentMemory }
    > = {}
