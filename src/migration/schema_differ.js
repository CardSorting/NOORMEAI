...
exports.generateSyncSQL = generateSyncSQL;
exports.applySchemaSyncronization = applySchemaSyncronization;
...
function compareSchemas(sourceSchema, targetSchema, sourceDialect, targetDialect) {
    ...
    for (const sourceTable of sourceSchema) {
        ...
        if (!targetTable) {
            ...
            // Compare columns
            const columnDiffs = compareColumns(sourceTable, targetTable, sourceDialect, targetDialect);
...
function compareColumns(sourceTable, targetTable, sourceDialect, targetDialect) {
    ...
    for (const sourceColumn of sourceTable.columns) {
        ...
        if (!targetColumn) {
            ...
            const compatible = (0, type_mapper_js_1.areTypesCompatible)(sourceType, targetType, sourceDialect, targetDialect);
            if (!compatible) {
                differences.push({
 ...
 */
function generateSyncSQL(comparison, targetDialect) {
    const sqlStatements = [];
    for (const diff of comparison.differences) {
        switch (diff.type) {
            case 'table_added':
                if (diff.details.source) {
                    const createTableSQL = generateCreateTableSQL(diff.details.source, targetDialect);
                    sqlStatements.push(createTableSQL);
                }
                break;
            case 'column_added':
                if (diff.details.source && diff.column) {
                    const addColumnSQL = generateAddColumnSQL(diff.table, diff.details.source, targetDialect);
                    sqlStatements.push(addColumnSQL);
                }
                break;
            case 'index_added':
                if (diff.details.source) {
                    const createIndexSQL = generateCreateIndexSQL(diff.details.source, targetDialect);
...
function generateAddColumnSQL(tableName, column, targetDialect) {
...
function generateCreateIndexSQL(index, targetDialect) {
 ...
 */
async function applySchemaSyncronization(targetDb, sqlStatements, options = {}) {
    const errors = [];
    ...
    for (const sqlStatement of sqlStatements) {
        ...
        try {
            await sql_js_1.sql.raw(sqlStatement).execute(targetDb);
            appliedStatements++;
        }
        catch (error) {
            errors.push({
