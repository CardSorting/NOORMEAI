...
export class DatabaseMigrationManager {
  ...
  async migrate(): Promise<MigrationResult> {
    ...
    try {
      ...
      this.logger.info(
        `   Source: ${this.config.source.dialect} (${this.config.source.database})`,
      )
      this.logger.info(
        `   Target: ${this.config.target.dialect} (${this.config.target.database})`,
      )
      ...
      this.logger.info(
        `   Filtered: ${filteredSourceSchema.length} tables to migrate`,
      )

      // Step 2: Migrate schema (if not data-only)
      if (!this.config.options?.dataOnly) {
        this.logger.info('\n🔧 Step 2: Migrating schema...')
        const schemaResult = await this.migrateSchema(
      ...
      const verificationResult =
        await this.verifyMigration(filteredSourceSchema)
  ...
  async syncSchema(options: SyncOptions = {}): Promise<{
    ...
    const sqlStatements = generateSyncSQL(
      comparison,
      this.config.target.dialect,
    )
    ...
    if (options.apply) {
      const result = await applySchemaSyncronization(
        this.targetDb,
        sqlStatements,
        options,
      )
  ...
  private filterTables(tables: TableSchema[]): TableSchema[] {
    ...
    if (
      this.config.options?.includeTables &&
      this.config.options.includeTables.length > 0
    ) {
      filtered = filtered.filter((t) =>
        this.config.options!.includeTables!.includes(t.name),
      )
    }

    if (
      this.config.options?.excludeTables &&
      this.config.options.excludeTables.length > 0
    ) {
      filtered = filtered.filter(
        (t) => !this.config.options!.excludeTables!.includes(t.name),
      )
    }
  ...
  private async migrateSchema(
      ...
      for (const table of targetSchema) {
        if (sourceSchema.find((t) => t.name === table.name)) {
          try {
            ...
            warnings.push(
              `Failed to drop table ${table.name}: ${(error as Error).message}`,
            )
          }
        }
      }
    ...
    for (const sourceTable of sourceSchema) {
      try {
        ...
        warnings.push(
          `Failed to create table ${sourceTable.name}: ${(error as Error).message}`,
        )
      }
    }
  ...
  private async migrateData(
    ...
    for (const result of results) {
      if (result.errors.length > 0) {
        warnings.push(
          `Table ${result.tableName} had ${result.errors.length} error(s)`,
        )
      }
   ...
   * Verify migration
   */
  private async verifyMigration(sourceSchema: TableSchema[]): Promise<{
    success: boolean
    warnings: string[]
  }> {
    const warnings: string[] = []
    ...
    for (const table of sourceSchema) {
      try {
        ...
        if (!verification.match) {
          ...
          this.logger.info(
            `     ✓ ${table.name}: ${verification.sourceCount} rows`,
          )
        }
      } catch (error) {
        warnings.push(
          `Failed to verify table ${table.name}: ${(error as Error).message}`,
        )
      }
    }
