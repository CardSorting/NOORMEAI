...
export class StrategicPlanner {
  ...
  async mutateStrategy(): Promise<string[]> {
    ...
    if (failedTests.length > 0) {
      console.warn(
        `[StrategicPlanner] Mutation cycle aborted. System health probes failed: ${failedTests.map((t) => t.name).join(', ')}`,
      )
      return []
    }
   ...
   */
  private async applyDirectMutation(
    ...
    return await this.db.transaction().execute(async (trx) => {
      ...
      console.log(
        `[StrategicPlanner] Applying direct mutation to persona ${persona.id} (Reason: ${reason})`,
      )
      ...
      for (const contradiction of contradictions) {
        if (updates.role && contradiction.includes(updates.role.slice(0, 20))) {
          console.warn(
            `[StrategicPlanner] Mutation blocked due to goal contradiction: ${contradiction}`,
          )
          return null
        }
      }
  ...
  private async verifyEvolution(persona: AgentPersona): Promise<string | null> {
    ...
    const rollbackHistory =
      (persona.metadata?.rollbackHistory as number[]) || []
    const recentRollbacks = rollbackHistory.filter(
      (ts) => Date.now() - ts < 604800000,
    ).length
    ...
    const hiveTrusted = allPersonas
      ...
      .filter(
        (p) =>
          p.metadata?.evolution_status === 'stable' &&
          p.metadata?.mutation_reason === persona.metadata?.mutation_reason,
      ).length
    ...
    if (earlyZ > 3.0 && earlyStats.sampleSize >= 5) {
      console.log(
        `[StrategicPlanner] High Performance Detected (Z=${earlyZ.toFixed(2)}). Accelerating stabilization threshold.`,
      )
    ...
    if (hiveTrusted >= 3) {
      console.log(
        `[StrategicPlanner] Accelerating verification for Persona ${persona.id} (Hive-Mind Trusted)`,
      )
    ...
    console.log(
      `[StrategicPlanner] Verifying persona ${persona.id}: Success=${report.successRate.toFixed(2)} (Baseline=${baseline.successRate.toFixed(2)}, σ=${stdDev.toFixed(3)}, Z-Score=${zScore.toFixed(2)}, Threshold=${sampleSizeThreshold})`,
    )

    // 1. Early Rollback (Critical statistical drop)
    if (zScore < -2.0) {
      console.warn(
        `[StrategicPlanner] STATISTICAL DEGRADATION detected for persona ${persona.id} (Z-Score: ${zScore.toFixed(2)}). Rolling back early.`,
      )
      return await this.rollbackPersona(persona.id)
    }
    ...
    if (report.sampleSize >= sampleSizeThreshold * 2 && zScore >= -0.5) {
      console.log(
        `[StrategicPlanner] Evolution for persona ${persona.id} STABILIZED.`,
      )
    ...
    const timeInVerification =
      (Date.now() - (persona.metadata?.verification_started_at || 0)) / 1000
    if (timeInVerification > 86400 * 3) {
      console.warn(
        `[StrategicPlanner] Verification period timed out for ${persona.id}. Rolling back to safety.`,
      )
      return await this.rollbackPersona(persona.id)
    }
  ...
  async analyzePersona(id: string | number): Promise<PerformanceReport> {
    ...
    const successMetrics = recentMetrics.filter(
      (m) => m.metric_name === 'task_success_rate',
    )
    const latencyMetrics = recentMetrics.filter(
      (m) => m.metric_name === 'query_latency',
    )
    ...
    console.log(
      `[StrategicPlanner] Analysis for ${id}: Success=${avgSuccess.toFixed(3)} (Min=${accuracyThreshold.toFixed(3)}), Latency=${avgLatency.toFixed(0)} (Max=${efficiencyThreshold.toFixed(0)})`,
    )
  ...
  async rollbackPersona(id: string | number): Promise<string> {
    return await this.db.transaction().execute(async (trx) => {
      ...
      const history =
        (persona.metadata?.mutationHistory as PersonaMutation[]) || []
