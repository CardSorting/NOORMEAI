...
export async function status(options: {
  ...
  try {
    ...
    console.log(
      chalk.gray(`  Relationships: ${schemaInfo.relationships.length}`),
    )
    console.log(
      chalk.gray(
        `  Total columns: ${schemaInfo.tables.reduce((sum, table) => sum + table.columns.length, 0)}`,
      ),
    )
    console.log(
      chalk.gray(
        `  Total indexes: ${schemaInfo.tables.reduce((sum, table) => sum + table.indexes.length, 0)}`,
      ),
    )
    console.log(
      chalk.gray(
        `  Total foreign keys: ${schemaInfo.tables.reduce((sum, table) => sum + table.foreignKeys.length, 0)}`,
      ),
    )
    ...
    console.log(
      chalk.gray(
        `  Auto-optimization: ${automationStatus.autoOptimization ? '✅ Enabled' : '❌ Disabled'}`,
      ),
    )
    console.log(
      chalk.gray(
        `  Query analysis: ${automationStatus.queryAnalysis ? '✅ Enabled' : '❌ Disabled'}`,
      ),
    )
    console.log(
      chalk.gray(
        `  Auto-indexing: ${automationStatus.autoIndexing ? '✅ Enabled' : '❌ Disabled'}`,
      ),
    )
    console.log(
      chalk.gray(
        `  Schema monitoring: ${automationStatus.schemaMonitoring ? '✅ Enabled' : '❌ Disabled'}`,
      ),
    )
    ...
    if (options.metrics !== false) {
      ...
      try {
        ...
        console.log(
          chalk[scoreColor](
            `  Overall Score: ${score}/100 ${score >= 80 ? '🟢' : score >= 60 ? '🟡' : '🔴'}`,
          ),
        )
        ...
        console.log(
          chalk.gray(`  Page count: ${metrics.pageCount.toLocaleString()}`),
        )
        console.log(
          chalk.gray(`  Free pages: ${metrics.freePages.toLocaleString()}`),
        )
    ...
    if (options.optimizations !== false) {
      ...
      try {
        ...
        if (optimizations.appliedOptimizations.length > 0) {
          console.log(
            chalk.green(
              `  ✅ ${optimizations.appliedOptimizations.length} optimizations applied:`,
            ),
          )
        ...
        console.log(
          chalk.gray(
            `    WAL mode: ${optimizations.walMode ? 'Enabled' : 'Disabled'}`,
          ),
        )
        console.log(
          chalk.gray(
            `    Foreign keys: ${optimizations.foreignKeys ? 'Enabled' : 'Disabled'}`,
          ),
        )
        console.log(chalk.gray(`    Auto vacuum: ${optimizations.autoVacuum}`))
        console.log(
          chalk.gray(`    Journal mode: ${optimizations.journalMode}`),
        )
        console.log(chalk.gray(`    Synchronous: ${optimizations.synchronous}`))
        console.log(
          chalk.gray(`    Cache size: ${optimizations.cacheSize} pages`),
        )
      } catch (error) {
        console.error(
          chalk.red('❌ Failed to get optimization status:'),
          error instanceof Error ? error.message : error,
        )
      }
    }
    ...
    if (options.cache !== false) {
      ...
      try {
        ...
        console.log(
          chalk.gray(`  Hit Rate: ${(cacheStats.hitRate * 100).toFixed(1)}%`),
        )
      } catch (error) {
        console.error(
          chalk.red('❌ Failed to get cache statistics:'),
          error instanceof Error ? error.message : error,
        )
      }
    }
    ...
    try {
      ...
      if (indexRecs.recommendations.length > 0) {
        console.log(
          chalk.yellow(
            `  📊 ${indexRecs.recommendations.length} recommendations available:`,
          ),
        )
        indexRecs.recommendations
          .slice(0, 5)
          .forEach((rec: IndexRecommendation, index: number) => {
            console.log(
              chalk.gray(
                `    ${index + 1}. ${rec.table}.${rec.column} - ${rec.reason}`,
              ),
            )
          })
        ...
        if (indexRecs.recommendations.length > 5) {
          console.log(
            chalk.gray(
              `    ... and ${indexRecs.recommendations.length - 5} more`,
            ),
          )
        }
      ...
      console.error(
        chalk.red('❌ Failed to get index recommendations:'),
        error instanceof Error ? error.message : error,
      )
    }
    ...
    console.log(chalk.blue.bold('\n🔄 Migration Status:'))
    try {
      const migrationManager = SQLiteMigrationManager.getInstance(
        db.getKysely(),
      )
      ...
      console.log(
        chalk.gray(`  Total migrations: ${migrationStatus.totalMigrations}`),
      )
      ...
      if (migrationStatus.lastMigration) {
        console.log(
          chalk.gray(`  Last migration: ${migrationStatus.lastMigration}`),
        )
        console.log(
          chalk.gray(
            `  Applied at: ${new Date(migrationStatus.lastAppliedAt!).toLocaleString()}`,
          ),
        )
      } else {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  Database file: ${(dbStats.size / 1024 / 1024).toFixed(2)}MB`,
        ),
      )
      console.log(
        chalk.gray(`  Last modified: ${dbStats.mtime.toLocaleString()}`),
      )
      ...
      try {
        ...
        console.log(
          chalk.gray(
            `  Generated files: ${generatedFiles.length} files in ./generated/`,
          ),
        )
      } catch {
        console.log(
          chalk.gray(`  Generated files: None (run 'npx noormme generate')`),
        )
      }
      ...
      try {
        ...
        console.log(
          chalk.gray(
            `  Migration files: ${migrationFiles.length} files in ./migrations/`,
          ),
        )
      } catch {
        console.log(
          chalk.gray(
            `  Migration files: None (run 'npx noormme migrate --generate')`,
          ),
        )
      }
    } catch (error) {
      console.error(
        chalk.red('❌ Failed to get file system status:'),
        error instanceof Error ? error.message : error,
      )
    }
  ...
  try {
    ...
    if (metrics.cacheHitRate < 0.8) {
      recommendations.push(
        'Consider increasing cache size - cache hit rate is low',
      )
    }
    ...
    if (indexRecs.recommendations.length > 0) {
      recommendations.push(
        `Apply ${indexRecs.recommendations.length} index recommendations`,
      )
    }
    ...
    try {
      ...
      if (migrationStatus.pendingMigrations > 0) {
        recommendations.push(
          `Apply ${migrationStatus.pendingMigrations} pending migrations`,
        )
      }
    } catch {
    ...
    const tablesWithoutIndexes = schemaInfo.tables.filter(
      (table: TableInfo) =>
        table.indexes.length === 0 && table.columns.length > 1,
    )
    if (tablesWithoutIndexes.length > 0) {
      recommendations.push(
        `Consider adding indexes to tables: ${tablesWithoutIndexes.map((t: TableInfo) => t.name).join(', ')}`,
      )
    }
  } catch (error) {
