  ...
  try {
    ...
    if (options.pragma !== false) {
      ...
      try {
        ...
        if (options.dryRun) {
          ...
          console.log(
            chalk.green(
              `✅ Applied ${result.appliedOptimizations.length} PRAGMA optimizations`,
            ),
          )
        }
      } catch (error) {
        console.error(
          chalk.red('❌ PRAGMA optimization failed:'),
    ...
    if (options.indexes !== false) {
      ...
      try {
        ...
        if (indexResult.recommendations.length > 0) {
          if (options.dryRun) {
            ...
            indexResult.recommendations.forEach((rec: any) => {
              console.log(
                chalk.gray(`  • ${rec.table}.${rec.column} (${rec.reason})`),
              )
            })
          } else {
            // Show index recommendations
            optimizationsApplied.push(
              ...indexResult.recommendations.map(
                (idx: any) =>
                  `Index recommendation: ${idx.table}.${idx.column}`,
              ),
            )
            console.log(
              chalk.green(
                `✅ Generated ${indexResult.recommendations.length} index recommendations`,
              ),
            )
          }
        } else {
        ...
        console.error(
          chalk.red('❌ Index optimization failed:'),
          error instanceof Error ? error.message : error,
        )
      }
    }
    ...
    if (options.analyze !== false) {
      ...
      try {
        if (options.dryRun) {
          console.log(
            chalk.gray('Would run ANALYZE to update query statistics'),
          )
        } else {
    ...
    if (options.wal !== false) {
      ...
      try {
        if (options.dryRun) {
          console.log(
            chalk.gray('Would enable WAL mode for better concurrency'),
          )
        } else {
          ...
          if (mode === 'wal') {
            ...
            console.log(
              chalk.yellow(
                `⚠️ WAL mode requested but returned mode is '${mode}'`,
              ),
            )
          }
        }
        ...
        console.error(
          chalk.red('❌ WAL mode configuration failed:'),
          error instanceof Error ? error.message : error,
        )
      }
    }
    ...
    console.log(chalk.blue('\n💡 Next Steps:'))
    console.log(
      chalk.gray('• Run "npx noormme analyze" to analyze query patterns'),
    )
    console.log(
      chalk.gray('• Use "npx noormme watch" for continuous optimization'),
    )
    console.log(
      chalk.gray('• Check "npx noormme status" to monitor performance'),
    )
